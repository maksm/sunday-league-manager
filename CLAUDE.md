...
Utility scripts in `scripts/` for database management:

- `setup-admin.ts` - Create admin user
- `check-password.ts` - Verify user password
- `reset-password.ts` - Reset user password
- `fix-inactive-players.ts` - Batch update player status
...
npx tsx scripts/check-password.ts <username> <password>
